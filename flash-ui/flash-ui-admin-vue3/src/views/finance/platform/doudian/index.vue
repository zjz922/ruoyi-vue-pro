<template>
    ...
    >
      ...
      <el-form-item>
        ...
        <el-button
          ...
          v-hasPermi="['finance:platform-config:create']"
        >
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list" :stripe="true" :show-overflow-tooltip="true">
      ...
      <el-table-column label="App Key" align="center" prop="appKey" width="150" />
      ...
      <el-table-column label="Token过期时间" align="center" prop="tokenExpireTime" width="180" :formatter="dateFormatter" />
      ...
      <el-table-column label="操作" align="center" fixed="right" width="280">
        <template #default="scope">
          <el-button
            ...
            v-hasPermi="['finance:platform-config:query']"
          >
            测试连接
          </el-button>
          <el-button
            link
            type="primary"
            @click="handleRefreshToken(scope.row.id)"
            v-hasPermi="['finance:platform-config:update']"
          >
            刷新Token
          </el-button>
          <el-button
          ...
          <el-button
            ...
            v-hasPermi="['finance:platform-config:delete']"
          >
            删除
          </el-button>
        </template>
  ...
  <!-- 表单弹窗：添加/修改 -->
  <DoudianConfigForm ref="formRef" @success="getList" />
</template>

<script setup lang="ts">
import { dateFormatter } from '@/utils/formatTime'
import * as PlatformConfigApi from '@/api/finance/platformConfig'
import DoudianConfigForm from './DoudianConfigForm.vue'

defineOptions({ name: 'DoudianConfig' })
...
const getList = async () => {
  loading.value = true
  try {
    const data = await PlatformConfigApi.getDoudianConfigPage(queryParams)
...
const handleTestConnection = async (id: number) => {
  try {
    const result = await PlatformConfigApi.testDoudianConnection(id)
    if (result) {
...
const handleRefreshToken = async (id: number) => {
  try {
    await PlatformConfigApi.refreshDoudianToken(id)
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await PlatformConfigApi.deleteDoudianConfig(id)

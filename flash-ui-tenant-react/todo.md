...
- GitHub仓库: https://github.com/zjz922/flash-saas.git
- 分支: master


## flash-saas框架集成开发（进行中）

### 正确理解
1. Vue3管理员端（flash-ui-admin-vue3）：框架已有，保持不变
2. 租户端（React）：作为flash-saas框架的一个前端模块集成进去（flash-ui-tenant-react）
3. Java后端：创建flash-module-finance模块，为租户端和管理员端提供API服务
4. 不改动框架现有的基础功能

### 待完成任务
- [ ] 检查flash-saas框架代码结构
- [ ] 创建Java后端财务模块flash-module-finance
- [ ] 集成租户端React项目到框架（flash-ui-tenant-react）
- [ ] 配置租户端与Java后端API对接
- [ ] 提交代码到GitHub

...
```
flash-saas-scaffold/
├── flash-dependencies/           # 依赖管理 (pom.xml)
├── flash-framework/              # 框架核心
│   ├── flash-common/             # 公共工具
│   ├── flash-spring-boot-starter-*/ # 各种Starter
│   └── ...
├── flash-server/                 # 服务器启动项
│   ├── pom.xml
│   └── src/main/java/...
...
│   └── src/main/java/...
├── flash-module-infra/           # 基础设施模块
├── flash-module-erp/             # ERP模块（参考）
├── flash-module-crm/             # CRM模块（参考）
├── flash-module-mall/            # 商城模块（参考）
├── flash-ui/                     # 前端UI
│   ├── flash-ui-admin-vue3/      # Vue3管理后台
│   ├── flash-ui-admin-vben/      # Vben管理后台
│   ├── flash-ui-admin-vue2/      # Vue2管理后台
│   └── flash-ui-mall-uniapp/     # 商城小程序
├── sql/                          # 数据库脚本
│   ├── mysql/
│   ├── postgresql/
│   └── ...
...
└── src/
    └── main/
        ├── java/
        │   └── cn/iocoder/flash/module/finance/
        │       ├── controller/
        │       │   └── admin/
        │       │       ├── doudian/
...
#### 1.1 创建模块目录结构
```bash
# 在flash-saas-scaffold中创建财务模块
mkdir -p flash-module-finance/src/main/java/cn/iocoder/flash/module/finance
mkdir -p flash-module-finance/src/main/resources/mapper/finance
```
...
<project xmlns="http://maven.apache.org/POM/4.0.0"
    ...
    <artifactId>flash-module-finance</artifactId>
    ...
    <dependencies>
        ...
        <dependency>
            <groupId>cn.iocoder.boot</groupId>
            <artifactId>flash-spring-boot-starter-mybatis</artifactId>
        </dependency>
        <!-- 其他依赖 -->
    </dependencies>
</project>
...
```
flash-module-finance/src/main/java/cn/iocoder/flash/module/finance/dal/dataobject/
```
...
```
flash-module-finance/src/main/java/cn/iocoder/flash/module/finance/dal/mysql/
```
...
```
flash-module-finance/src/main/resources/mapper/finance/
```

#### 2.4 创建Service接口和实现
在以下目录创建Service类：
```
flash-module-finance/src/main/java/cn/iocoder/flash/module/finance/service/
```
...
在以下目录创建Controller类：
```
flash-module-finance/src/main/java/cn/iocoder/flash/module/finance/controller/admin/
```
...
#### 3.2 配置数据源
在`flash-server/src/main/resources/application.yml`中配置：
```yaml
spring:
  datasource:
    url: jdbc:mysql://localhost:3306/flash?useSSL=false&serverTimezone=UTC
    username: root
    password: password
...
#### 5.1 配置应用属性
在`flash-server/src/main/resources/application.yml`中添加：
```yaml
# 财务模块配置
finance:
  doudian:
...
# 启动
mvn spring-boot:run -pl flash-server
```
...
| 源文件 | 目标位置 | 说明 |
|--------|--------|------|
| ruoyi-integration/java-complete/entity/*.java | flash-module-finance/src/main/java/.../dal/dataobject/ | Entity类 |
| ruoyi-integration/java-complete/mapper/*.java | flash-module-finance/src/main/java/.../dal/mysql/ | Mapper接口 |
| ruoyi-integration/java-complete/mapper-xml/*.xml | flash-module-finance/src/main/resources/mapper/finance/ | Mapper XML |
| ruoyi-integration/java-complete/service/*.java | flash-module-finance/src/main/java/.../service/ | Service接口和实现 |
| ruoyi-integration/java-complete/controller/*.java | flash-module-finance/src/main/java/.../controller/admin/ | Controller类 |
| ruoyi-integration/java-complete/vo/*.java | flash-module-finance/src/main/java/.../controller/admin/*/vo/ | VO类 |
| ruoyi-integration/java-complete/config/*.java | flash-module-finance/src/main/java/.../framework/config/ | 配置类 |
...
| flash-ui/flash-ui-admin-vue3/src/views/finance/ | 新建 | Vue3管理员端 |
| flash-ui/flash-ui-admin-vue3/src/api/finance/ | 新建 | Vue3 API |
...
### 5.1 Entity类示例
```java
package cn.iocoder.flash.module.finance.dal.dataobject.order;

import cn.iocoder.flash.framework.mybatis.core.dataobject.BaseDO;
...
### 5.2 Mapper接口示例
```java
package cn.iocoder.flash.module.finance.dal.mysql.order;

import cn.iocoder.flash.framework.mybatis.core.mapper.BaseMapper;
import cn.iocoder.flash.module.finance.dal.dataobject.order.OrderDO;
...
### 5.3 Service接口示例
```java
package cn.iocoder.flash.module.finance.service.order;

import cn.iocoder.flash.module.finance.dal.dataobject.order.OrderDO;
import cn.iocoder.flash.module.finance.controller.admin.order.vo.*;
...
### 5.4 Service实现示例
```java
package cn.iocoder.flash.module.finance.service.order.impl;

import cn.iocoder.flash.module.finance.dal.dataobject.order.OrderDO;
import cn.iocoder.flash.module.finance.dal.mysql.order.OrderMapper;
import cn.iocoder.flash.module.finance.service.order.OrderService;
...
### 5.5 Controller示例
```java
package cn.iocoder.flash.module.finance.controller.admin.order;

import cn.iocoder.flash.framework.common.pojo.CommonResult;
import cn.iocoder.flash.framework.common.pojo.PageResult;
import cn.iocoder.flash.module.finance.controller.admin.order.vo.*;
import cn.iocoder.flash.module.finance.convert.order.OrderConvert;
import cn.iocoder.flash.module.finance.dal.dataobject.order.OrderDO;
import cn.iocoder.flash.module.finance.service.order.OrderService;
...
### 6.3 Vue3管理员端环境变量
```bash
# flash-ui/flash-ui-admin-vue3/.env
VITE_API_BASE_URL=http://localhost:48080/app-api
